...
class PacManController(o: Observer?) : GameEntryPoint(o,
                                                      WindowConfig.alexander_title) {
    private val changer: PacManFxmlChanger
        ...
        if (arg is String) {
            when (arg) {
                ...
                UpdateCodes.PacMan.mainMenu      -> {
                    ...
                    changer.changeFxml(pacManMenu,
                                       UpdateCodes.PacMan.mainMenu)
                    exitToMainGUI()
                }
                UpdateCodes.PacMan.showEndScreen -> showEndScreen()
                UpdateCodes.PacMan.repeatGame    ->                     //changer.changeFxml( pacManEndScreen, UpdateCodes.PacMan.showEndScreen );
                    startGame()
                else                             -> logParsingError(
                        o,
                        arg)
            }
    ...
    init {
        changer = PacManFxmlChanger(this,
                                    PacManMenu.fxml,
                                    pacManMenu)
    }
}
